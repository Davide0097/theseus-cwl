...
import { Workflow, Input, CWLPackedDocument } from "@theseus-cwl/types";
...
export type WorflowProviderProps = {
  ...
  initialCwlObject: Workflow | CWLPackedDocument;
  initialColorState?: ColorState;
};

export const WorkflowProvider = (props: WorflowProviderProps) => {
  const [cwlObject, setCwlObject] = useState<Workflow | CWLPackedDocument>(
   ...
   * They must not be considered part of the public API.
   */
  // const updateStep = (id: string, updatedData: Partial<WorkflowStep & HasKey>) => {
  //   try {
  //     setCwlObject((prev) => {
